...
| Script | Purpose |
|--------|---------|
| [`paw`](paw) | Main CLI wrapper for the plan/edit/implement/diagnose/tighten/review/prototype workflow plus lightweight repo teaching, architecture review, PR/issue helpers, lint, compact, archive, migration, terminal task browsing, local GUI, and setup. `paw plan` seeds one task package in the central task store by default, investigates repo landmark files directly, and records the current branch/worktree assignment when inside Git. `paw implement` saves a checksum-verified binary patch, baseline/index contract and owned paths when provenance is safe; `paw implement-batch <task-name>...` launches multiple eligible approved tasks concurrently while preserving the normal single-task path for each child. `paw review <task-name>` seeds `review.md` and runs a task-quality review with reviewed scope, grade, optional overall workflow/subsystem grade, threshold comparison, architectural/design notes, production-readiness blockers, and recommendations. `paw prototype <task-name>` requires that review record, creates a `<task-name>-prototype` plan-only replacement package, records prototype lineage/status metadata, and reverses the saved patch only after checksum, content/mode, ownership and baseline-index checks; legacy path-only provenance blocks cleanup; missing provenance or mixed work leaves the replacement plan in place and records cleanup as blocked or unavailable. `paw browse <task-name>` opens the resolved task package's Markdown docs in a terminal pager, resolving central packages before legacy `.agent/<task>/` packages; `PAW_BROWSE_PAGER=cat` forces deterministic stdout. `paw archive <task-name>` moves a central task package under `.archive/` so active CLI and GUI task lists omit it. `paw gui` foregrounds the localhost task control surface on port `8765` by default, with `--port 0` available for an explicit ephemeral port; the dashboard includes newest-first task ordering, safe Markdown rendering, local auto-refresh for index, detail, and live-log views, active repo add/select controls, hidden-by-default state/repo-text/completion filters, task-name detail links, Archive-first row controls, selected-task Archive/Delete actions, queued Plan prompts, `plan.md` dashboard preview, approval previews for implementation-ready tasks, `View PR` lookup for saved PAW branches that still exist locally with distinct auth/configuration, no-PR, invalid-URL, and CLI diagnostics, instruction overlays, `Answer Questions` edit overlays that pass blocked-task answer text to `paw edit`, GUI implementation launches without extras after approval, `Stream` plus Cancel for verified live PAW run metadata with matching task-local GUI logs, `Use as Prototype` controls plus prototype markers and cleanup messages, guarded task deletion, high-grade Prototype disabling, and an Archived dashboard with guarded Unarchive; `paw gui start` records managed background PID/URL metadata, `paw gui restart` replaces only the recorded process while reusing recorded mode metadata unless options override, `paw gui stop`/`kill` stop only that recorded process, and `paw gui --all` shows every central task store grouped by repo identity while the active repo selector controls new Plan targets. `paw teach` is a lighter-weight top-level explainer that maps the relevant modules and callers for an unfamiliar area using repo vocabulary, without creating a dedicated task package or silently writing durable docs. `paw architecture` explores the repo first, saves numbered candidates under `.agent/architecture/candidates.md`, and resumes the selected candidate via `--pick <n>` plus `.agent/architecture/grill.md`. `paw diagnose` reuses the same approved task package and branch/worktree safety rules as `paw implement`, but adds a feedback-loop-first debugging workflow with reviewable debug notes in `plan.md`. `paw tighten` stays plan-only, sharpens an existing task package one question at a time, and seeds `tighten.md` as the interactive checkpoint while leaving `plan.md` as the actual plan source of truth. `paw edit`, `paw implement`, `paw diagnose`, `paw tighten`, `paw prototype`, `paw to-issues`, `paw pr-submit`, `paw pr-review`, and `paw issue-submit` try to resume the saved assignment when safe. For full subcommand behavior and examples, see [`examples/docs/cli-reference.md`](../examples/docs/cli-reference.md). |
| [`install-paw.sh`](install-paw.sh) | Makefile helper for conservative checkout-owned launcher install/uninstall; refuses foreign destinations and validates executable sources. |
