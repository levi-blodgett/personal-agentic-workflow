...
- For blocked cleanup, inspect the saved patch and both staged and unstaged diffs, preserve unrelated changes, and manually remove only reviewed source work. Do not add owned-path metadata to bypass the checks. Keep the worktree idle during capture/cleanup; Git worktree edits are not transactionally locked. After replacement implementation, run `paw review <replacement-task>` and request an overall review/prototype workflow grade against the B+ threshold.
