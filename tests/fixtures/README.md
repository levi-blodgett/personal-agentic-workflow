...
|-----------|-------|---------|
| `sample-task-valid/` | `contract.md`, `plan.md`, and optional `pr.md` present; all required sections exist in canonical order; `## Current Status` contains the required status fields. | `lint-task.bats` (valid-task pass), `list-tasks.bats`, CI smoke (`scripts/lint-task.sh tests/fixtures/sample-task-valid`). |
