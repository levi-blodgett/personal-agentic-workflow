...
| `task-store.bats` | central task-store resolution, metadata, legacy fallback, and migration helpers |
| `gui-server.bats` | `paw gui` local dashboard smoke coverage |
...
- Task-store and GUI tests set `PAW_TASK_HOME` to `$BATS_TEST_TMPDIR` so central-store behavior is hermetic and never writes to the operator's real local state.
