...
| `gui-server.bats` | `paw gui` Markdown rendering, newest-first task sorting, auto-refresh fragment plumbing, main-table filters, active repo registry/selector behavior, combined repo/branch display, home navigation, concise action labels, overlay prompts/previews, no-extra GUI implementation, local plan/edit/implement/review/prototype/archive actions, click-time View PR lookup for saved live branches, selected-task archive/delete, prototype markers, guarded deletion, foreground smoke, managed start/stop/restart/kill lifecycle, stable default-port coverage, stale PID protection, and multi-repo dashboard coverage |
